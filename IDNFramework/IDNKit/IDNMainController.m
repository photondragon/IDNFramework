...
@implementation IDNMainController
...
- (void)initialize
{
	if(_mainController==nil)
	{
		...
		_mainController.view.translatesAutoresizingMaskIntoConstraints = NO;
		_mainController.view.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
		_mainController.view.layer.shadowOpacity = 1.0;
		[self.view addSubview:_mainController.view];
		
		maskView = [[UIControl alloc] initWithFrame:CGRectMake(slideMax, 0, SideViewRightMargin, size.height)];
//		maskView.backgroundColor = [UIColor colorWithRed:0 green:1.0 blue:0 alpha:0.3];
...
- (void)addSideView
{
	if(_sideController)
	{
		if(_sideController.view.superview)//已经添加
			return;
		CGSize size = self.view.frame.size;
		_sideController.view.frame = CGRectMake(0, 0, size.width-SideViewRightMargin, size.height);
		_sideController.view.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
		[self.view insertSubview:_sideController.view atIndex:0];
	}
}
- (void)removeSideView
{
...
//			NSLog(@"test:%@",[NSValue valueWithCGPoint:touchStartPoint]);
	...
	else if(panGesture.state==UIGestureRecognizerStateChanged)
	{
		...
		if(isSliding)
		{
			...
			mainFrame.origin.x = slideLength;
