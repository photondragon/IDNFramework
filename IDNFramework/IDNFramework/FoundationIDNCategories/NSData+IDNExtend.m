...
- (NSString*)crc32
{
	...
	return [NSString stringWithFormat:@"%u", (unsigned int)crc];
}

- (void)createCRC32Table //创建crc32表
{
	UInt32 table[256];
	...
	for (int i=0; i<32; i++) {
		for (int j = 0; j<8; j++) {
			[s appendString:[NSString stringWithFormat:@"0x%08x,", (unsigned int)table[i*8+j]]];
		}
		[s appendString:@"\n"];
	}
	[s appendString:@"}"];
